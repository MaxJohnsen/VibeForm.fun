-- Function to get form stats for multiple forms in one query
CREATE OR REPLACE FUNCTION get_form_stats(p_form_ids UUID[])
RETURNS TABLE (
  form_id UUID,
...
BEGIN
  week_start := date_trunc('week', now()) - interval '5 weeks';
  ...
  SELECT 
    ...
    COALESCE(wa.weekly_arr, ARRAY[0,0,0,0,0,0]::BIGINT[]) AS weekly_responses
  FROM unnest(p_form_ids) AS f(id)
